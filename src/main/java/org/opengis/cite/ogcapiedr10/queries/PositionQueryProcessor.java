...
public class PositionQueryProcessor extends AbstractProcessor {
	...
	public String validatePositionQueryUsingParameters(Set<String> collectionIds, String rootUri, int noOfCollections,
		...
		for (int c = 0; c < maximum; c++) {
			...
			if (supportsPositionQuery == false) { // Avoids Nullpointer Exception
				sb.append(" The position element is missing from the data_queries element of the collection "
						+ collectionId + " .");
			}
			...
			if (supportsPositionQuery) {
				...
				for (int f = 0; f < outputFormatList.size(); f++) {
					if (outputFormatList.get(f).equals("CoverageJSON")
							|| outputFormatList.get(f).contains("CoverageJSON")) { // preference
																					// for
																					// CoverageJSON
																					// if
																					// supported
				...
				String constructedURL = url + "/position?parameter-name=" + sampleParamaterNameSafe + "&coords="
						+ "POINT(" + medianx + "+" + mediany + ")" + "&crs=" + supportedCRS + "&f=" + supportedFormat
				...
				if (pageContent != null) {
					...
					sb.append("Response of Position Query on collection " + collectionId + " was null. \n");
				}
