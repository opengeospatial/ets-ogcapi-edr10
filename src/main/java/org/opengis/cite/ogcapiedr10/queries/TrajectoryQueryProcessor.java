...
public class TrajectoryQueryProcessor extends AbstractProcessor {
	...
	public String validateTrajectoryQueryUsingParameters(Set<String> collectionIds, String rootUri, int noOfCollections,
		...
		for (int c = 0; c < maximum; c++) {
			...
			if (supportsTrajectoryQuery == false) { // Avoids Nullpointer Exception
				sb.append(" The trajectory element is missing from the data_queries element of the collection "
						+ collectionId + " .");
			}
			...
			if (supportsTrajectoryQuery) {
				...
				String constructedURL = url + "/trajectory?parameter-name=" + sampleParamaterNameSafe + "&coords="
						+ "LINESTRING(" + lminx + "+" + lminy + "," + medianx + "+" + mediany + "," + lmaxx + "+"
						+ lmaxy + ")" + "&f=" + supportedFormat + "&datetime=" + sampleDateTime;
				...
				if (pageContent != null) {
					...
					sb.append("Response of Trajectory Query on collection " + collectionId + " was null. \n");
				}
