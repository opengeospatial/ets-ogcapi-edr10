...
public class AreaQueryProcessor extends AbstractProcessor {
	...
	public String validateAreaQueryUsingParameters(Set<String> collectionIds, String rootUri, int noOfCollections,
		...
		for (int c = 0; c < maximum; c++) {
			...
			if (supportsAreaQuery) {
				...
				if (jsonResponse.getJsonObject("extent") == null) { // Avoids Nullpointer
																	// Exception
				...
				String constructedURL = url + "/area?parameter-name=" + sampleParamaterNameSafe + "&coords="
						+ "POLYGON((" + lminx + "+" + lminy + "," + lminx + "+" + lmaxy + "," + lmaxx + "+" + lmaxy
						+ "," + lmaxx + "+" + lminy + "," + lminx + "+" + lminy + "))" + "&crs=" + supportedCRS + "&f="
				...
				if (pageContent != null) {
					...
					sb.append("Response of Area Query on collection " + collectionId + " was null. \n");
				}
